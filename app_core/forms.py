"""Form builder for capturing apartment details."""
...
from app_core.form_helpers import (
    checkbox,
    number_input_float,
    number_input_int,
    optional_select,
...
def _validate_required_numbers(payload: Dict[str, Any]) -> List[str]:
    """Return a list of validation errors for required numeric fields."""

    errors: List[str] = []
    if payload["price_input_value"] <= 0:
...
def build_apartment_form() -> Optional[Dict[str, Any]]:
    ...
    with st.form("apartment_form"):
        ...
        transaction_type = st.radio(
            "Transaction Type", ["rent", "sale"], horizontal=True, help="rent or sale"
        )
        posted_date = st.date_input("Posted Date", value=date.today())
        scraped_date = st.date_input("Scraped Date", value=date.today())
        data_source = optional_text("Data Source", help_text="e.g., ikman.lk")
        ...
        with col_lat:
            latitude = number_input_float(
                "Latitude", min_value=-90.0, step=0.0001, format_str="%.6f"
            )
        with col_lng:
            longitude = number_input_float(
                "Longitude", min_value=-180.0, step=0.0001, format_str="%.6f"
            )
        ...
        price_input_value = number_input_float(
            "Price (lakhs for rent, millions for sale)",
            min_value=0.0,
            step=0.1,
            format_str="%.2f",
        )
        ...
        maintenance_fee_lkr = number_input_int(
            "Maintenance fee (LKR)", min_value=0, value=0
        )
        maintenance_fee_per_sqft = number_input_float(
            "Maintenance fee per sqft", min_value=0.0, step=0.01, format_str="%.2f"
        )
        other_monthly_fees_lkr = number_input_int(
            "Other monthly fees (LKR)", min_value=0, value=0
        )
        rent_frequency = optional_select(
            "Rent frequency", ["per_month", "per_day", "per_week", "per_year"]
        )
        ...
        with col3:
            size_sqft = number_input_int(
                "Size (sqft)", min_value=1, value=1, step=10
            )
            balcony_count = number_input_int("Balcony count", min_value=0, value=0)
            expected_completion_year = number_input_int(
                "Expected completion year", min_value=0, step=1, value=0
            )
        ...
        with col_le1:
            min_lease_months = number_input_int(
                "Min lease months", min_value=0, value=0
            )
            key_money_months = number_input_float(
                "Key money (months)", min_value=0.0, step=0.1
            )
            electricity_separate = checkbox("Electricity separate")
            furnished_included_in_rent = checkbox("Furnished included in rent")
        with col_le2:
            max_lease_months = number_input_int(
                "Max lease months", min_value=0, value=0
            )
            advance_months = number_input_float(
                "Advance (months)", min_value=0.0, step=0.1
            )
            water_separate = checkbox("Water separate")
            short_term_allowed = checkbox("Short term allowed")
        with col_le3:
            maintenance_included_in_rent = checkbox(
                "Maintenance included in rent"
            )
            security_deposit_months = number_input_float(
                "Security deposit (months)", min_value=0.0, step=0.1
            )
            agent_fee_months = number_input_float(
                "Agent fee (months)", min_value=0.0, step=0.1
            )
            short_term_min_nights = number_input_int(
                "Short term minimum nights", min_value=0, value=0
            )
    ...
    errors = _validate_required_numbers(payload)
    if errors:
    ...
    payload["price_lkr"] = int(payload["price_input_value"] * price_multiplier)
